    ...
    for dyad in dyads:
        ...
        for year in years:
            ...
            rows.append({
                ...
                "us_gdp_current_usd": us_gdp if not pd.isna(us_gdp) else np.nan,
                "target_gdp_current_usd": t_gdp if not pd.isna(t_gdp) else np.nan,
                "us_military_expenditure_usd": us_mil if not pd.isna(us_mil) else np.nan,
                "target_military_expenditure_usd": t_mil if not pd.isna(t_mil) else np.nan,
                "us_trade_percent_gdp": us_trade if not pd.isna(us_trade) else np.nan,
                "target_trade_percent_gdp": t_trade if not pd.isna(t_trade) else np.nan,
            })
...
def add_context_features(monthly_df: pd.DataFrame, use_worldbank: bool = True) -> pd.DataFrame:
    ...
    # Raw World Bank columns are useful for reporting. They remain NaN when
    # World Bank is disabled or unavailable, and the model loader imputes them.
    for col in [
