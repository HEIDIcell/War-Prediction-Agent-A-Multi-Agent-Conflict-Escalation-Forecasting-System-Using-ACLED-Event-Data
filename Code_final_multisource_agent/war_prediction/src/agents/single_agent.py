...
class SingleRAGAgent:
    ...
    def predict_one(self, row: pd.Series, model_probability: float):
        ...
        article_titles = []
        if hasattr(self.retriever, "retrieve_articles"):
            try:
                article_titles = self.retriever.retrieve_articles(str(row.get("dyad", "")), str(row.get("month", "")), k=2)
            except Exception:
                article_titles = []
        gdelt_text = ""
        if article_titles:
            gdelt_text = " Retrieved GDELT news evidence includes: " + "; ".join(article_titles[:2]) + "."
